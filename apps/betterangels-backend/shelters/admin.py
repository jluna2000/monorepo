...
class ShelterAdmin(ImportExportModelAdmin):
    ...
    def get_readonly_fields(
        ...
        )  # Done to avoid error with Mypy, triggers warning since Admin now inherits from ImportExportModelAdmin, module has no type stubs
        if not request.user.has_perm(ShelterFieldPermissions.CHANGE_IS_REVIEWED):
            readonly_fields = (*readonly_fields, "is_reviewed")
        return readonly_fields

    # following block overwrites default method behavior to handle skipped rows for import processing
